...
Apply the fix by running: sudo python3 apply_web_motor_fix_cleaned.py
"""
import os
import sys
import time
...
def fix_motor_control():
    ...
    try:
        ...
        # Fix 8: Fix atexit registration to ensure proper parameters and GPIO import
        print("- Fixing atexit registration")
        atexit_original = "import atexit\natexit.register(cleanup_motors)"
        atexit_fixed = "import atexit\natexit.register(lambda: cleanup_motors(reset_gpio=False))\n"
...
python3 apply_web_motor_fix_cleaned.py
...
def cleanup_unwanted_test_scripts():
    """Remove unnecessary test scripts to avoid confusion."""
    print_section("Cleaning up unwanted test scripts")
    ...
    try:
        # Get all test_*.py files in the directory
        directory = os.path.dirname(os.path.abspath(__file__))
        test_files = [f for f in os.listdir(directory) if f.startswith('test_') and f.endswith('.py')]
        
        # Create backup directory
        backup_dir = os.path.join(directory, 'old_tests_backup')
        if not os.path.exists(backup_dir):
            os.makedirs(backup_dir)
            print(f"Created backup directory: {backup_dir}")
        
        # Move unwanted test files to backup
        for test_file in test_files:
            if test_file not in tests_to_keep:
                src_path = os.path.join(directory, test_file)
                dst_path = os.path.join(backup_dir, test_file)
                shutil.move(src_path, dst_path)
                print(f"Moved {test_file} to backup")
        
        print("Cleanup complete.")
        return True
        
    except Exception as e:
        print(f"Error cleaning up test scripts: {e}")
        return False

def main():
    """Main function."""
    print("=== Web Motor Interface Fix (Cleaned) ===")
    print("This script will fix the issue with motors working in test scripts but not in the web interface.")
    
    # Clean up unwanted test scripts
    cleanup_unwanted_test_scripts()
