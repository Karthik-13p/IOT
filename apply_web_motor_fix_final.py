...
Apply the fix by running: sudo python3 apply_web_motor_fix_final.py
"""
import os
import sys
import time
...
def cleanup_app():
    """
        ...
        # Find the cleanup function in the content
        cleanup_start_idx = content.find(cleanup_func)
        if cleanup_start_idx != -1:
            # Find the end of the function
            cleanup_end_idx = content.find("\n\n", cleanup_start_idx)
            if cleanup_end_idx == -1:
                cleanup_end_idx = len(content)
...
def cleanup_app():
    ...
    try:
        ...
        print(f"Error during web app cleanup: {e}")"""
            ...
            content = content[:cleanup_start_idx] + cleanup_func_fixed + content[cleanup_end_idx:]
...
def emergency_stop():
    """
        ...
        # Find the emergency stop function in the content
        emergency_start_idx = content.find(emergency_stop_func)
        if emergency_start_idx != -1:
            # Find the end of the function
            emergency_end_idx = content.find("\n\n", emergency_start_idx)
            if emergency_end_idx == -1:
                emergency_end_idx = len(content)
...
def emergency_stop():
    ...
    try:
        ...
        return jsonify({"status": "error", "message": str(e)})"""
            ...
            content = content[:emergency_start_idx] + emergency_stop_func_fixed + content[emergency_end_idx:]
...
def fix_motor_control():
    ...
    try:
        ...
        # Look for the initialize_motors function definition
        init_pattern = re.compile(r'def initialize_motors\(.*?\):\n.*?""".*?"""', re.DOTALL)
        match = init_pattern.search(content)
        if match:
            init_func = match.group(0)
            init_func_fixed = init_func.replace(
                'global motors_initialized, motor_pwm\n', 
                'global motors_initialized, motor_pwm\n    \n    # Use thread lock for thread safety\n    with motor_lock:\n'
            )
            content = content.replace(init_func, init_func_fixed)
        
        # Fix 3: Add thread safety to set_motor_speed function
        print("- Adding thread safety to motor speed control")
        
        # Look for the set_motor_speed function definition
        set_speed_pattern = re.compile(r'def set_motor_speed\(.*?\):\n.*?""".*?""".*?\n    if not motors_initialized:', re.DOTALL)
        match = set_speed_pattern.search(content)
        if match:
            set_speed_func = match.group(0)
            set_speed_func_fixed = set_speed_func.replace(
                'global motors_initialized, motor_pwm\n', 
                'global motors_initialized, motor_pwm\n    \n    # Use thread lock for thread safety\n    with motor_lock:\n'
            )
            content = content.replace(set_speed_func, set_speed_func_fixed)
        
        # Fix 4: Add thread safety to cleanup_motors function
        print("- Adding thread safety to motor cleanup")
        
        # Look for the cleanup_motors function definition
        cleanup_pattern = re.compile(r'def cleanup_motors\(reset_gpio=False\):\n.*?""".*?""".*?global motors_initialized, motor_pwm', re.DOTALL)
        match = cleanup_pattern.search(content)
        if match:
            cleanup_func = match.group(0)
            cleanup_func_fixed = cleanup_func + '\n    \n    # Use thread lock for thread safety\n    with motor_lock:'
        ...
        # Look for the move_forward function definition
        forward_pattern = re.compile(r'def move_forward\(speed=100\):\n.*?""".*?""".*?\n    if not motors_initialized', re.DOTALL)
        match = forward_pattern.search(content)
        if match:
            forward_func = match.group(0)
            forward_func_fixed = forward_func.replace(
                'print(f"Moving forward at speed {speed}")\n', 
                'print(f"Moving forward at speed {speed}")\n    \n    # Use thread lock for thread safety\n    with motor_lock:\n'
            )
            content = content.replace(forward_func, forward_func_fixed)
        
        # Fix 6: Add thread safety to move_backward function
        print("- Adding thread safety to backward movement")
        
        # Look for the move_backward function definition
        backward_pattern = re.compile(r'def move_backward\(speed=100\):\n.*?""".*?""".*?\n    if not motors_initialized', re.DOTALL)
        match = backward_pattern.search(content)
        if match:
            backward_func = match.group(0)
            backward_func_fixed = backward_func.replace(
                'print(f"Moving backward at speed {speed}")\n', 
                'print(f"Moving backward at speed {speed}")\n    \n    # Use thread lock for thread safety\n    with motor_lock:\n'
            )
            content = content.replace(backward_func, backward_func_fixed)
        
        # Fix 7: Add thread safety to stop function
        print("- Adding thread safety to motor stop")
        
        # Look for the stop function definition
        stop_pattern = re.compile(r'def stop\(\):\n.*?""".*?""".*?\n    if not motors_initialized', re.DOTALL)
        match = stop_pattern.search(content)
        if match:
            stop_func = match.group(0)
            stop_func_fixed = stop_func.replace(
                'print("Stopping all motors")\n', 
                'print("Stopping all motors")\n    \n    # Use thread lock for thread safety\n    with motor_lock:\n'
            )
            content = content.replace(stop_func, stop_func_fixed)
...
'''
    ...
    try:
        ...
        os.chmod(test_path, 0o755)  # Make it executable
...
python3 apply_web_motor_fix_final.py
...
def main():
    """Main function."""
    print("=== Web Motor Interface Fix (Final) ===")
